...
const createDefaultSEO = (canonical: string): SEOProps => ({
	...
	description: "Portfolio d'un développeur web FullStack, basé dans les alentours de marseille",
	canonical,
	openGraph: {
		basic: {
			title: "Kev Stfnl",
...
const mergeOpenGraph = (base?: SEOProps["openGraph"], extra?: SEOProps["openGraph"]) => {
	...
	return {
		...
		optional: base?.optional || extra?.optional ? { ...base?.optional, ...extra?.optional } : undefined,
		image: base?.image || extra?.image ? { ...base?.image, ...extra?.image } : undefined,
		article: base?.article || extra?.article ? { ...base?.article, ...extra?.article } : undefined,
	};
};
...
const mergeTwitter = (base?: SEOProps["twitter"], extra?: SEOProps["twitter"]) => {
	if (!base && !extra) return undefined;
	return {
		...(base ?? {}),
		...(extra ?? {}),
...
export const buildSEOProps = (canonical: string, overrides?: SEOProps): SEOProps => {
